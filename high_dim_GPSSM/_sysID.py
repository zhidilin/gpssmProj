...
Details:
    - The script construct the EnVI-GPSSM model using separate GPs for the transition function.
    - The script uses the EnKF for the inference.
...
for _, data_name in enumerate(data_name_all):
    ...
    DIR = f'results/SysID/EnVI/{data_name}/fixEmission_{fixEmission}_fixTrans_{fixTransition}_eNoise_{round(observation_noise_sd, 3)}_tNoise_{round(process_noise_sd, 3)}_consistentELBO_{sampleU}/'
