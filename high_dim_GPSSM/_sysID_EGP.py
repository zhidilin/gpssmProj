"""
This is a script for running experiments on the System Identification datasets using EnVI-GPSSM model.

Compared to the _sysID.py, the main differences are:
    - The script construct the EnVI model using a 'Shared GP + Normalizing Flows' for the transition function modeling
"""
import os
import time
import torch
...
from modules.EGPSSM import EnVI as GPSSMs
# from modules.EGPSSM import RealNVP_EnVI as GPSSMs
...
for _, data_name in enumerate(data_name_all):
    ...
    DIR = f'results/SysID/EGPSSM/{data_name}_BNN_{if_BNN}_pureNN_{if_pureNN}/fixEmission_{fixEmission}_fixTrans_{fixTransition}_eNoise_{round(observation_noise_sd, 3)}_tNoise_{round(process_noise_sd, 3)}_consistentELBO_{sampleU}/'
    ...
    for jj in range(num_repeat):
        ...
        # uniformly generate inducing points from [-2, 2],  shape: number_ips x (state_dim + input_dim)
        ips = 4 * torch.rand((number_ips, state_dim+input_dim)) - 2

        ############    Model preparation
        model = GPSSMs(dim_x=state_dim, dim_y=output_dim, seq_len=seq_len, ips=ips, dim_c=input_dim,
                       ...
                       residual_trans=False).to(device)
        if if_pureNN:
