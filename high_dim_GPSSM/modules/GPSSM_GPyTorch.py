...
from .gpModel import GP_Module, _cholesky_factor
...
class EnVI(nn.Module):
    def __init__(self, dim_x, dim_y, seq_len, ips, dim_c = 0, N_MC = 50, process_noise_sd=0.05, emission_noise_sd=0.1,
        ...
        self.transition = GP_Module(inducing_points=ips,  batch_size=self.state_dim)
    ...
    def forward(self, observations,  input_sequence=None, state_init=None):
        ...
        if state_init is None:
            ...
            qm0_KL = torch.tensor([0.], device=observations.device, dtype=observations.dtype)
            x_t_1 = state_init.unsqueeze(0).unsqueeze(0).repeat(self.N_MC, self.state_dim, 1, 1)
    ...
    def iterate_sequence(self, observations, x_0, input_sequence=None):
        ...
        if self.consistentSampling:
            induc_induc_covar = self.transition.kernel(self.transition.inducing_points).add_jitter().evaluate()
            L = torch.linalg.cholesky(induc_induc_covar)

            ''' #################    sample U, i.e., U ~ q(U)  ################# '''
            # shape [state_dim x num_ips x (state_dim + input_dim)]
            ips = self.transition.inducing_points
    ...
    def GP_Predict(self, x_t_1, L=None, U=None, input_sequence=None,
                   localization_radius=None, linear_obs=True, var_inflation = 0.0):
        """

        Parameters
        ----------
        x_t_1:              shape: N_MC x state_dim x batch_size x state_dim
        L:                  shape: state_dim x num_ips x num_ips
        ...
        if self.consistentSampling:
            # sampling u first, then condition on u
            # GPdynamics is N_MC predictive means.  Its shape: N_MC x state_dim x batch_size
            GPdynamics = self.transition.condition_u(gp_input, U, L)

        else:
            # 👻 marginalize out U (slower in running speed, but the performance is the not big difference)
            GPdynamics = self.transition(gp_input, L).rsample()   # MultivariateNormal, shape: N_MC x state_dim x batch_size

        if self.residual_trans:
            bias =  x_t_1.data[:, 0].transpose(-1,-2)
            x_t = GPdynamics + bias                    # shape: N_MC x state_dim x batch_size
        else:
            x_t = GPdynamics
    ...
    def GP_Predict_forcasting(self, x_t_1, L=None, U=None, input_sequence=None,
                              localization_radius=None, linear_obs=True, var_inflation = 0.0):
        """
        This function implements the GP prediction step for the EnKF. It is used to forecast the state of the system
        one time step ahead.
        ...
        if self.consistentSampling:
            ...
            # 👻 marginalize out U (slower in running speed, but the performance is the not big difference)
            GPdynamics = self.transition(gp_input, L).mean       # MultivariateNormal, shape: N_MC x state_dim x batch_size
